###########################################################################################################
# analysis.py Yfinance with sentiment analysis_ Corporate Actions_ Erarnings
import pandas as pd
...
def rank_stocks(data, news_api_key, alpha_vantage_key):
    ...
    for symbol, df in data.items():
        if df is not None and not df.empty:
            ...
            earnings_trend = corporate_actions['earnings_trend'] if corporate_actions else 'No Data'
...
def save_ranking_to_csv(ranking_df):
    try:
        ...
        logging.info("Ranking data saved to CSV as 'stock_ranking.csv'.")
    except PermissionError:
        logging.error("Permission denied: 'stock_ranking.csv'. Trying with a different file name.")
        ranking_df.to_csv('stock_ranking_backup.csv', index=False)
        logging.info("Ranking data saved to CSV as 'stock_ranking_backup.csv'.")
...
def save_ranking_to_html(ranking_df):
    ...
    logging.info("Ranking data saved to HTML as 'stock_ranking.html'.")


###########################################################################################################

# # analysis.py Yfinance with sentiment analysis_ Corporate Actions_ Erarnings _ With Retry
